...
TONE_GUIDANCE = (
    ...
    "קליניות. חובה לבדוק כל פרט באופן עצמאי מול המקור.'\n"
    "Say it EXACTLY as written — do not add or substitute words. End with 'מול "
    "המקור'. Never say 'מול המקור המקורי' (redundant and grating), and do not "
    "narrow it to 'המאמר' — sources are not always articles.\n"
    "\n"
    ...
    "\n"
    "========================================================================\n"
    "FIDELITY — the mistakes that actually happen. Do not make them:\n"
    "========================================================================\n"
    "1. ONLY the papers in the source exist. Never introduce a study, a "
    "journal name, an author, a dose, an effect size or a statistic that "
    "is not in the source — not as an example, not as supporting "
    "evidence, not in passing. A whole fictitious paper, its journal and "
    "its AUC values were once presented as this week's research.\n"
    "   You SHOULD still bring general psychiatric knowledge in to "
    "explain and connect, and you should say plainly when a paper here "
    "updates, differs from or contradicts accepted practice — that is "
    "what makes the review worth listening to. Mark it as general "
    "knowledge when you do ('ידוע ש...', 'בניגוד למקובל, המחקר הזה "
    "מוצא ש...') and give it no citation. The listener must never be "
    "left unsure whether something came from this week's papers.\n"
    ...
    # Rules 6-9 come from the QC trends report over 52 episodes (six
    # occurrences of distorted numbers and reversed meanings, four of mangled
    # researcher names, three of breaking character over missing material) and
    # from the misattributions that held dulcan-031 and kaplan-029 in the books
    # project. Approved by the owner 2026-09-23.
    "6. NUMBERS AND NEGATIONS, EXACTLY AS WRITTEN. Strictly adhere to the "
    "factual data and numbers in the source. Pay special attention to decimal "
    "points and to negating words ('not', 'no', 'independent', 'unchanged'): "
    "dropping one reverses the finding. Never invert a logical relationship, "
    "and never mix findings, methods or numbers between different papers.\n"
    "7. NAMES AS PRINTED. Do not translate, localise or alter a researcher's "
    "name, and never guess a first name from an initial. Use the exact surname "
    "as printed in the source and transliterate it into Hebrew phonetically "
    "and faithfully.\n"
    "8. NEVER BREAK THE FOURTH WALL to say what you were given — no "
    "mentioning that only an abstract, a title or a partial text was provided, "
    "and no speculating about why something is missing. Teach what is in front "
    "of you. Where the source genuinely stops short, say plainly that the "
    "point is not reported, and move on.\n"
    "9. ATTRIBUTE TO THE SOURCE ONLY WHAT IT ACTUALLY SAYS. Phrases like "
    "'המחקר מראה', 'לפי המאמר' or 'החוקרים מצאו' are a claim about the source, "
    "not a figure of speech: use them only for something that is genuinely "
    "there. Correct general knowledge presented as if it came from this week's "
    "papers is still an error — mark it as general knowledge instead.\n"
    "\n"
...
def _spotlight_prompt(a: dict, review_channel_he: str = "") -> str:
    ...
    return (
        ...
        "Treat this as a LONG, comprehensive single-paper episode.\n"
        "\n"
        "HOW TO DIVIDE THE EPISODE (important — keep roughly to this):\n"
        "  ~15% question and background · ~25% methodology · ~35% results · "
        "~25% interpretation and clinical implications.\n"
        "Methodology is the SETUP, not the destination. Explain it clearly and "
        "simply — the listener must be able to judge how much to trust the "
        "result — then MOVE ON. Do not walk through every design detail for its "
        "own sake. The results and what they mean are the heart of the episode "
        "and must get more airtime than the methods, not less.\n"
        "\n"
        "Cover, ADAPTING to the study type:\n"
        "  • The clinical or scientific question the paper addresses.\n"
        "  • The methodology, concisely and in plain language: enough to judge "
        "the strength of the evidence. Prioritise the few design choices that "
        "actually determine how much to believe the result (for a trial: what "
        "was compared with what, blinding, the primary outcome; for a "
        "meta-analysis: what was pooled and how heterogeneous it was; for a "
        "cohort: sample, follow-up, and the confounders that matter; for a "
        "guideline: the strength of evidence behind each recommendation). "
        "Mention the rest only if it changes the interpretation.\n"
        "  • THE RESULTS, in depth — this is the core of the episode. The "
        "primary outcome with the actual numbers and effect size, what it means "
        "in absolute terms (how many patients, how much change), the secondary "
        "outcomes, adverse events and who dropped out, subgroups where the "
        "effect differed, and how precise or uncertain each number is. Walk "
        "through the main findings one by one — do not summarise them in a "
        "sentence or two.\n"
        "  • THE DISCUSSION, in depth: what the authors conclude and whether "
        "the data actually support it, how this fits or clashes with what was "
        "believed until now, the controversies and counter-arguments, and the "
        "limitations that genuinely constrain the conclusion.\n"
